...
func TestRoundRobinSelectorCycles(t *testing.T) {
	...
	for i, want := range expected {
		got := selector.Select(WorkItem{}, 3)
		if got != want {
...
func TestSourceIPHashSelectorStableForClient(t *testing.T) {
	...
	item := WorkItem{RemoteAddr: "203.0.113.10"}
...
func TestSourceIPHashSelectorNormalizesHostPort(t *testing.T) {
	...
	hostOnly := selector.Select(WorkItem{RemoteAddr: "198.51.100.42"}, 7)
	hostPort := selector.Select(WorkItem{RemoteAddr: "198.51.100.42:5555"}, 7)
