...
func TestCompareDetailedDetectsDiff(t *testing.T) {
	primary := http.Header{}
	shadow := http.Header{}
	primary.Set("Auth-Status", "OK")
	shadow.Set("Auth-Status", "FAIL")

	_, _, diffs, diff := CompareDetailed(primary, shadow, []string{"Auth-Status"}, false)
	if !diff {
...
func TestWriteSelected(t *testing.T) {
	...
	src := http.Header{}
	src.Set("Auth-Status", "OK")
	src.Set("X-Unrelated", "skip")

	WriteSelected(rec, src, []string{"Auth-Status"})
